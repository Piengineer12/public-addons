...
PANEL = {
    Init: => -- this is the only function that's called automatically on the parent too
        ...
        with @insaneStats_HealthItemLine = vgui.Create 'InsaneStats_ShopItemLine', @
            \SetText 'Health'
            .OnPollDetails = (_, id) ->
                ply = LocalPlayer!
                ...
                freeQty = 0
                if (IsValid(shopEntity) and not hook.Run 'InsaneStatsBlockFreebie', ply, shopEntity, 257)
                    freeQty = math.max 0, ply\InsaneStats_GetMaxHealth! - ply\InsaneStats_GetHealth!

                local qty, price
                if id > 4
                    fullSpend = coins / unitPrice
                    price = math.max 0, switch id
                        when 5 then fullSpend / 100
                        when 6 then fullSpend / 10
                        when 7 then fullSpend
        ...
        with @insaneStats_ArmorItemLine = vgui.Create 'InsaneStats_ShopItemLine', @
            \SetText 'Shield'
            .OnPollDetails = (_, id) ->
                ply = LocalPlayer!
                coins = ply\InsaneStats_GetCoins!
                ...
                shopEntity = @GetShopEntity!
                freeQty = 0
                if (IsValid(shopEntity) and not hook.Run 'InsaneStatsBlockFreebie', ply, shopEntity, 258)
    ...
    SetAmmoSold: (ammoIDs) =>
        ...
        for details in *allDetails
            with vgui.Create 'InsaneStats_ShopItemLine', @
