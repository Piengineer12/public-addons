...
function ENT:AcceptInput(input,activator,caller,data)
	...
	elseif input:lower()=="altermode" or input:lower()=="setmode" then
		self.SetMode = data
	...
	elseif input:lower()=="getcashmin" or input:lower()=="getcashmax" then
		...
		local amt = cashtable[math.Clamp(tonumber(data) or 1,1,#cashtable)]
		if isMin then
	...
	elseif input:lower()=="altercash" or input:lower()=="setcash" then
		if tonumber(self.SetMode) == 0 then
			ROTGB_SetCash(data,activator)
			self:TriggerOutput(self.OnCashSet,activator,ROTGB_GetCash(activator))
		elseif tonumber(self.SetMode) == 1 then
			ROTGB_AddCash(data,activator)
			self:TriggerOutput(self.OnCashSet,activator,ROTGB_GetCash(activator))
		elseif tonumber(self.SetMode) == 2 then
			ROTGB_RemoveCash(data,activator)
			self:TriggerOutput(self.OnCashSet,activator,ROTGB_GetCash(activator))
		elseif tonumber(self.SetMode) == 3 then
			ROTGB_SetCash(data*player.GetCount())
			self:TriggerOutput(self.OnCashSet,activator,ROTGB_GetCash(activator))
		elseif tonumber(self.SetMode) == 4 then
			ROTGB_AddCash(data*player.GetCount())
			self:TriggerOutput(self.OnCashSet,activator,ROTGB_GetCash(activator))
		elseif tonumber(self.SetMode) == 5 then
			ROTGB_RemoveCash(data*player.GetCount())
			self:TriggerOutput(self.OnCashSet,activator,ROTGB_GetCash(activator))
		elseif tonumber(self.SetMode) == 6 then
			ROTGB_SetCash(data)
			self:TriggerOutput(self.OnCashSet,activator,ROTGB_GetCash(activator))
		elseif tonumber(self.SetMode) == 7 then
			ROTGB_AddCash(data)
			self:TriggerOutput(self.OnCashSet,activator,ROTGB_GetCash(activator))
		elseif tonumber(self.SetMode) == 8 then
