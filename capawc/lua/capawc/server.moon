...
minBlendTimeConVar = CreateConVar 'capawc_sv_minimum_blend_time', '0', FCVAR_ARCHIVE,
	'Minimum time between color blends. This is ignored when "blending" two colors that are the same.', 0, 2
botConVar = CreateConVar 'capawc_sv_bots', '1', FCVAR_ARCHIVE,
	'Should bots be affected? Note that bots will simply have rainbow colors.', 0, 1
gamemodeBlacklistConVar = CreateConVar 'capawc_sv_gamemode_blacklist', '', FCVAR_ARCHIVE,
	'Player colors will never be animated in these gamemodes. This has no effect on weapon colors.'
gamemodeWhitelistConVar = CreateConVar 'capawc_sv_gamemode_whitelist', 'base sandbox cinema elevator jazztronauts', FCVAR_ARCHIVE,
	'Player colors will be animated in these gamemodes when the gamemode whitelist is enabled.'
gamemodeWhitelistEnabledConVar = CreateConVar 'capawc_sv_gamemode_whitelist_enabled', 0, FCVAR_ARCHIVE,
	'Player colors will only be animated in the gamemodes specified by capawc_sv_gamemode_whitelist.'
...
cvars.AddChangeCallback 'capawc_sv_minimum_blend_time', ((name, oldValue, newValue) ->
...
hook.Add 'Think', 'capawc', ->
	for ply in *player.GetAll!
		if not ply\IsBot! or botConVar\GetBool!
			...
			if animationData and gamemodeEnables and tobool ply\GetInfo 'capawc_cl_player_colors_enabled'
				color = GetCurrentColorByAnimationData animationData
				ply\SetPlayerColor color
			animationData = weaponAnimationPlayerData[ply]
			if animationData and tobool ply\GetInfo 'capawc_cl_weapon_colors_enabled'
				color = GetCurrentColorByAnimationData animationData
