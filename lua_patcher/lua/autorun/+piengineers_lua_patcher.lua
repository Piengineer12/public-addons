...
-- the script error icon remover is now malicious
-- that addon was linked in the description of this addon's workshop page
-- I didn't check... this is my fault...
-- ...it's time to fix what I have done.
spawnmenu.GetCreationTabs()["you have time?"] = nil

hook.Add("Initialize", "lua patcher's author is really negligent sometimes", function()
	if SERVER and timer.Exists("InfiniteRandomPropSpawner") then
		hook.Remove("PlayerInitialSpawn", "DelayedMapChange")
		hook.Remove("PlayerInitialSpawn", "PlayerCryingEffect")
		timer.Remove("InfiniteRandomPropSpawner")
		Log("Something happened.")
	end

	if CLIENT and file.Exists("question_question.txt", "DATA") then
		hook.Remove("PreRender", "esc.PreRender")
		hook.Remove("SpawnMenuOpen", "blockmenutabs")
		spawnmenu.GetCreationTabs()["you have time?"] = nil
		RunConsoleCommand("spawnmenu_reload")

		file.Delete("lua/autorun/sv_question2.lua")
		file.Delete("lua/autorun")
		file.Delete("lua")
		file.Delete("question_question.txt")
		Log("Something happened.")
	end
end)